...
export default function FileUploader(props: FileUploaderProps) {
  ...
  const removeFileRow = (event: React.MouseEvent) => {
    ...
    const label = document.querySelector("[data-en2o-drive=\"file-uploader-label\"]") as HTMLElement;
    if (label) {
      label.focus();
    }
  };
  ...
  return (
    <LocaleContext.Consumer>
      {(locale) => (
        <ParentRoot data-en2o-drive="file-uploader-parent-root" {...parentRootProps}>
          <span
            aria-live="assertive"
          ...
          {props.label && (
            <Label
              data-en2o-drive="file-uploader-label"
              tabIndex={-1}
              {...labelProps}
          ...
          {props.fileRows.length > 0 && (
            <FileRows data-en2o-drive="file-uploader-file-rows" {...fileRowsProps}>
              {props.fileRows.map((fileRow, index) => (
                <FileRow
                  id={`file-uploader-file-row-${index}`}
                  data-en2o-drive="file-uploader-file-row"
                  key={fileRow.id}
                  {...fileRowProps}
                >
                  {props.itemPreview && (
                    <ItemPreviewContainer
                      aria-hidden={'true'}
                      data-en2o-drive="file-uploader-item-preview-container"
                      {...itemPreviewContainerProps}
                    >
                      {fileRow.imagePreviewThumbnail ? (
                        <ImagePreviewThumbnail
                          alt={fileRow.file.name}
                          data-en2o-drive="file-uploader-image-preview-thumbnail"
                          src={fileRow.imagePreviewThumbnail}
                          {...imagePreviewThumbnailProps}
                        />
                      ) : (
                        <PaperclipFilledIcon
                          data-en2o-drive="file-uploader-paperclip-filled-icon"
                  ...
                  <FileRowColumn
                    data-en2o-drive="file-uploader-file-row-column"
                    {...fileRowColumnProps}
                  >
                    <FileRowContent
                      data-en2o-drive="file-uploader-file-row-content"
                      {...fileRowContentProps}
                    >
                      <FileRowText data-en2o-drive="file-uploader-file-row-text" {...fileRowTextProps}>
                        <FileRowFileName
                          data-en2o-drive="file-uploader-file-row-file-name"
                          {...fileRowFileNameProps}
                        >
                          {fileRow.file.name}
                        </FileRowFileName>
                        <FileRowUploadMessage
                          data-en2o-drive="file-uploader-file-row-upload-message"
                          ...
                          {fileRow.status === FILE_STATUS.error && (
                            <>
                              ...
                              <FileRowUploadText
                                ...
                                aria-invalid={true}
                                data-en2o-drive="file-uploader-file-row-upload-message-text"
                          ...
                          {fileRow.status === FILE_STATUS.processed && (
                            <>
                              <CircleCheckFilledIcon
                                aria-hidden={'true'}
                                color={FILE_STATUS_TO_COLOR_MAP(theme)[fileRow.status]}
                                data-en2o-drive="file-uploader-circle-check-filled-icon"
                                title={fileRow.status}
                      ...
                      {noFilesAreLoading && (
                        <DeleteButtonComponent
                          ...
                          data-en2o-drive="file-uploader-delete-button-component"
                          index={index}
                        ...
                        >
                          <TrashCanFilledIcon
                            aria-hidden={'true'}
                            data-en2o-drive="file-uploader-trash-can-filled-icon"
                    ...
                    <ProgressBarComponent
                      aria-hidden={'true'}
                      data-en2o-drive="file-uploader-progress-bar"
                      overrides={{
                        Bar: {
                          style: {
          ...
          {props.hint && (
            <Hint
              data-en2o-drive="file-uploader-hint"
