...
export default (context: vscode.ExtensionContext) => {
  ...
  function updateDecorations() {
    ...
    const workspaceConfig = vscode.workspace.getConfiguration("en2o-drive");
  ...
  vscode.workspace.onDidChangeConfiguration((event) => {
    if (
      event.affectsConfiguration("en2o-drive.theme.coloring.enabled") ||
      event.affectsConfiguration("en2o-drive.theme.coloring.source") ||
      event.affectsConfiguration("en2o-drive.theme.coloring.style")
    ) {
      triggerUpdateDecorations();
    }
  }),
